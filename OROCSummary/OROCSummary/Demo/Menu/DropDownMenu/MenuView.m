...
- (void)setUpUIWithFrame:(CGRect)frame{
    ...
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, kNavBarHeight, self.target.view.bounds.size.width, self.target.view.bounds.size.height)];
...
+ (MenuView *)createMenuWithFrame:(CGRect)frame target:(UIViewController *)target dataArray:(NSArray *)dataArray itemsClickBlock:(void (^)(NSString *, NSInteger))itemsClickBlock backViewTap:(void (^)())backViewTapBlock{
    ...
    CGFloat y = frame.origin.y ? frame.origin.y : kNavBarHeight - KMargin * 0.5;
