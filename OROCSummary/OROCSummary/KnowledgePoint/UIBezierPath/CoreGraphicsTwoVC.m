...
//    _titleArr = @[@"画多边形",@"画矩形", @"创建圆形或者椭圆形", @"创建一段弧线", @"在矩形中针对四角中的某个角加圆角", @"以某个中心点画弧线", @"画二元曲线一般和moveToPoint配合使用", @"以三个点画一段曲线"];
    _titleArr = @[@"画多边形",@"画矩形", @"创建圆形或者椭圆形", @"创建一段弧线", @"绘制二次贝塞尔曲线", @"绘制三次贝塞尔曲线", @"画带圆角的矩形", @"指定矩形的某角为圆角", @"ShapeImageVC"];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        ShapeImageVC *rounding = [[ShapeImageVC alloc] init];
        [self navigaitonControllerPush:rounding titleName:@"ShapeImageVC"];
    }
}
